...
def login():
    if request.method == "POST":
        ...
        if not email or not bio_file:
            flash("Email and Biometric Image are required.", "danger")
            return redirect(url_for("login"))

        # Save temp image
        temp_name = f"login_{secrets.token_hex(8)}.jpg"
        temp_path = os.path.join(app.config["UPLOAD_FOLDER"], temp_name)
        bio_file.save(temp_path)
        ...
        try:
            ...
            # keep or clean up; for audit trail we keep minimal
            pass
